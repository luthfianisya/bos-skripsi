            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id}>
                {headerGroup.headers.map((header) => {
                  const isSticky = header.column.id === "grup" || header.column.id === "aksi"  || header.column.id === "checkbox";
                  return (
                    <TableHead
                          key={header.id}
                          colSpan={header.colSpan}
                          className={
                            header.column.id === "grup" || header.column.id === "checkbox" || header.column.id === "aksi"
                          ...
                          style={
                            header.column.id === "checkbox"
                              ...
                              ? { left: 50, width: 150, minWidth: 150 } // posisi setelah checkbox
              ...
              table.getRowModel().rows.map((row) => (
                <TableRow key={row.id} data-state={row.getIsSelected() && "selected"} className="group hover:bg-muted">
                  {row.getVisibleCells().map((cell) => {
                    const isSticky = cell.column.id === "grup" || cell.column.id === "aksi"  || cell.column.id === "checkbox";
                    return (
                      <TableCell
                        key={cell.id}
                        className={`${
