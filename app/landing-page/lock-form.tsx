"use client";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import avatar from "@/public/images/avatar/avatar-8.jpg";
import { useState } from "react";
import { Label } from "@/components/ui/label";
import { Input } from "@/components/ui/input";
import { Icon } from "@iconify/react";
import { Button } from "@/components/ui/button";
import Link from "next/link";
import { SiteLogo } from "@/components/svg";
import { useMediaQuery } from "@/hooks/use-media-query";
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip";
const LockForm = () => {
  const [passwordType, setPasswordType] = useState("password");
  const isDesktop2xl = useMediaQuery("(max-width: 1530px)");
  return (
    <div className="w-full">
      <div className="flex justify-center">
        <Link href="/dashboard" className="inline-block">
          <SiteLogo className="h-10 w-10 2xl:h-14 2xl:w-14 text-primary" />
        </Link>
      </div>
      <div className="2xl:mt-8 mt-4 2xl:text-3xl text-2xl font-bold text-default-900 text-center">
        BackOffice Selindo
      </div>
      <div className="2xl:text-lg text-base text-default-600 mt-2 leading-6 text-center">
        {/* Enter the information you entered while registering. */}
      </div>

      <div className="mt-6 flex justify-center">
        <Avatar className="h-[72px] w-[72px]">
          <AvatarImage src={avatar.src} alt="" />
          <AvatarFallback>SN</AvatarFallback>
        </Avatar>
      </div>
      <div className="text-center mt-4 text-xl font-medium text-default-900">
        Aldi Pratama
      </div>

      <form className="2xl:mt-7 mt-8">
        <div className="mt-6">
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Link href="/administrator/dashboard" className="w-full">
                  <Button className="w-full" size={!isDesktop2xl ? "lg" : "md"} color="primary">
                    Login SSO BPS
                  </Button>
                </Link>
              </TooltipTrigger>
              <TooltipContent color="secondary">
                <p>Khusus untuk pegawai BPS</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>

        {/* Login SSO Eksternal */}
        <div className="mt-6">
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <span className="w-full">
                  <Button
                    className="w-full"
                    size={!isDesktop2xl ? "lg" : "md"}
                    color="primary"
                    variant="outline"
                  >
                    Login SSO Eksternal
                  </Button>
                </span>
              </TooltipTrigger>
              <TooltipContent color="secondary">
                <p>Fitur belum tersedia</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
      </form>
      {/* <div className="mt-8 text-center text-base font-medium text-default-600">
        Not you? Return{" "}
        <Link href="/auth/login4" className="text-primary">
          Sign In
        </Link>
      </div> */}
    </div>
  );
};

export default LockForm;
